...
Type objective_function<Type>::operator() ()
{
  ...
  if( Options_vec(2)==1){
    for(int r=0; r<n_r; r++){
    for(int t=0; t<n_t; t++){
      encounterprob_rt(r,t) = plogis( densvar_z(1) + densvar_z(2)*ln_u_gt(r,t) );
    }}
  }
  ...
  for(int i=0; i<n_i; i++){
    ...
    if( Options_vec(2)==1){
      ...
      NLL_i(i) = -1 * dzinflognorm(c_i(i), ln_u_gt(r_i(i),t_i(i)), encounterprob_rt(r_i(i),t_i(i)), exp(densvar_z(0)), true);
    }
  }
  NLL_c(2) = NLL_i.sum();
  NLL = NLL_c.sum();
