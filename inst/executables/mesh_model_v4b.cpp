...
Type objective_function<Type>::operator() ()
{
  ...
  // Objective function
  vector<Type> NLL_c(4);    // 0:ln_u; 1:Epsilon; 2:Likelihood
  NLL_c.setZero();
  ...
  // First year
  for(int r=0; r<n_r; r++){
    ln_uhat_gt(r,0) = logmeanu0 + log(km2_r(r)) + Omega_g(r);
    u_rt(r,0) = exp( ln_u_gt(r,0) );
    // fishing mortality and catch
    catchpred_t(0) += exp(ln_uhat_gt(r,0)) * (1-exp(-1 *effortdens_rt(r,0) * F_t(0)));
  ...
  for(int t=1; t<n_t; t++){
    ...
    // Fishing mortality and catch
    for(int r=0; r<n_r; r++){
      catchpred_t(t) += upred_rt(r,t) * (1-exp(-1 * effortdens_rt(r,t) * F_t(t)));
    ...
    for(int r=0; r<n_r; r++){
      if(Options_vec(1)==0) ln_uhat_gt(r,t) = log( upred_rt(r,t) * exp(alpha + log(km2_r(r)) + Omega_g(r) - beta*log(upred_rt(r,t))) );
      if(Options_vec(1)==1) ln_uhat_gt(r,t) = log( upred_rt(r,t) * exp(alpha + log(km2_r(r)) + Omega_g(r) - beta*upred_rt(r,t)) );
    }
    for(int g=n_r; g<n_g; g++) ln_uhat_gt(g,t) = 0.0;
  }
  
  //array<Type> encounterprob_rt(n_r,n_t);
  // Calculate encounter probability (only used for delta-lognormal model)
  //if( Options_vec(2)==1){
  //  for(int r=0; r<n_r; r++){
  //  for(int t=0; t<n_t; t++){
  //    if( Options_vec(3)==0 ) encounterprob_rt(r,t) = plogis(densvar_z(1) + densvar_z(2)*ln_u_gt(r,t) );
  //    if( Options_vec(3)==1 ) encounterprob_rt(r,t) = plogis(densvar_z(1)) * ( 1.0 - exp(-1 * u_rt(r,t)*exp(densvar_z(2))) );
  //    if( Options_vec(3)==2 ) encounterprob_rt(r,t) = plogis(densvar_z(3)) * plogis( densvar_z(1) + densvar_z(2)*ln_u_gt(r,t) );
  //  }}
  //}
  ...
  for(int i=0; i<n_i; i++){
    ...
    if( Options_vec(2)==1){
      //encounterprob_i(i) = encounterprob_rt( r_i(i), t_i(i) );
      ...
      NLL_i(i) = -1 * dzinflognorm(c_i(i), log(chat_i(i))-log(encounterprob_i(i)), encounterprob_i(i), exp(densvar_z(0)), true);
    }
  }
  NLL_c(2) = NLL_i.sum();
