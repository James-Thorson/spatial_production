...
Type objective_function<Type>::operator() ()
{
  ...
  for(int r=0; r<n_r; r++){
    ...
    catchpred_t(0) += exp(ln_uhat_gt(r,0)) * (1-exp(-1 *effort_rt(r,0) * F_t(0)));
    ln_uhat_gt(r,0) = ln_uhat_gt(r,0) - ( effort_rt(r,0) * F_t(0) );
  }
  ...
  for(int t=1; t<n_t; t++){
    ...
    // Fishing mortality and catch
    for(int r=0; r<n_r; r++){
      catchpred_t(t) += upred_rt(r,t) * (1-exp(-1 * effort_rt(r,t) * F_t(t)));
      upred_rt(r,t) = upred_rt(r,t) * exp(-1 * effort_rt(r,t) * F_t(t));
    }
  ...
  for(int i=0; i<n_i; i++){
    ...
    if( Options_vec(2)==1){
      ...
      NLL_i(i) = -1 * dzinflognorm(c_i(i), ln_u_gt(r_i(i),t_i(i))/encounterprob_rt(r_i(i),t_i(i)), encounterprob_rt(r_i(i),t_i(i)), exp(densvar_z(0)), true);
    }
  }
  NLL_c(2) = NLL_i.sum();
  ...
  // Catch penalty
  for(int t=0; t<n_t; t++){
    NLL_c(3) -= dnorm( log(catch_t(t)), log(catchpred_t(t)), exp(ln_sigmacatch), true );
  }
