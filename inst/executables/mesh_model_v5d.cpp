...
Type dzinfgamma(Type x, Type mean, Type encounter_prob, Type cv, int give_log=false){
  ...
  if(x==0){
    ...
    pos_prob = dgamma( x, pow(cv,-2), mean*pow(cv,2), false );
    joint_prob = encounter_prob * pos_prob;
  } 
  if(give_log) return log(joint_prob); else return joint_prob;
}
...
Type objective_function<Type>::operator() ()
{
  ...
  for(int i=0; i<n_i; i++){
    ...
    if( Options_vec(2)==1 | Options_vec(2)==2){
      ...
      if( Options_vec(2)==1 ) NLL_i(i) = -1 * dzinflognorm(c_i(i), log(chat_i(i))-log(encounterprob_i(i)), encounterprob_i(i), exp(densvar_z(0)), true);
